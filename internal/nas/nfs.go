...
func (n *NFS) Mount(ctx context.Context) (Mounts, func() error, error) {
	...
	for _, s := range specs {
		...
		if err := os.MkdirAll(target, 0o755); err != nil {
			cleanupErr := unmount()
			return Mounts{}, func() error { return nil }, errors.Join(fmt.Errorf("create mount point %s: %w", target, err), cleanupErr)
		}
		...
		out, err := n.runMount(ctx, opts, src, target)
		if err != nil {
			cleanupErr := unmount()
			exit := 0
			var ee *exec.ExitError
			...
			return Mounts{}, func() error { return nil }, errors.Join(fmt.Errorf("mount %s: %w", s.name, ClassifyMountError(string(out), exit)), cleanupErr)
		}
